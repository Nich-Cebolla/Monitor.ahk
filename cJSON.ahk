...
class JSON {
	 ...
	 */
	static stringify(obj, expandlevel := unset, LFCR := true, space := "  ") {
        result := Trim(CO(obj, IsSet(expandlevel) ? Abs(expandlevel) : 10000000))
		return (LFCR ? RegExReplace(result,'\R','`r`n') : result)
		CO(O, J := 0, R := 0, Q := 0) {
    ...
    static SimpleClone(obj) => JSON.parse(JSON.stringify(obj), , false)
    static SimpleCloneToMap(obj) => JSON.parse(JSON.stringify(obj))
}
